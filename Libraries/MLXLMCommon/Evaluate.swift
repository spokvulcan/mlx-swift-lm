...
protocol TokenIteratorProtocol: Sequence, IteratorProtocol where Element == Int {
    ...
    /// The KV cache state owned by the iterator. For speculative iterators this
    /// returns the main model's cache (the draft cache is not exposed at this
    /// level). Used by ``FinalizedKVCacheHandle`` to recover the post-generation
    /// cache for prefix-cache reuse paths.
...
public struct SpeculativeTokenIterator: TokenIteratorProtocol {
    ...
    /// `TokenIteratorProtocol` conformance: expose the main model's cache.
    /// The draft cache is intentionally not surfaced here — it is short-lived
    /// scratch state that the prefix-cache consumer cannot meaningfully reuse.
    var cache: [KVCache] { mainCache }
...
// tesseract: tools parameter plumbed in from our generation call sites so
// schema-aware type conversion in tool-call parsers actually fires. See
// /Users/owl/.claude/plans/fix-the-triattention-sparse-kv-sparkling-porcupine.md
// — upstream re-sync should preserve the default-nil signature.
