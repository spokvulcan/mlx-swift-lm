...
/// The compiled walk matches the eager one for a float32 pairing and for the
/// shipped one: a bfloat16 drafter beside a float16 ParoQuant target, whose
/// embedding and hidden states the drafter casts at its entries so the fused
/// residual norm, dynamic conv and greedy walk see one dtype.
@Test(arguments: [
    (drafter: DType.float32, target: DType.float32),
    (drafter: DType.bfloat16, target: DType.float16),
])
func testDFlash2CompiledProposalMatchesEager(pairing: (drafter: DType, target: DType)) throws {
    ...
    let parameters = compiled.parameters().mapValues { $0.asType(pairing.drafter) }
    try compiled.update(parameters: parameters, verify: [])
    try eager.update(parameters: parameters, verify: [])
    let target = MockTarget()
    try target.embedding.update(
        parameters: target.embedding.parameters().mapValues { $0.asType(pairing.target) },
        verify: [])
    ...
    for (round, spec) in rounds.enumerated() {
        if round == 1 || round == 4 {
            ...
            let updated = DFlash2DraftModel(config).parameters().mapValues {
                $0.asType(pairing.drafter)
            }
            try compiled.update(parameters: updated, verify: [])
            try eager.update(parameters: updated, verify: [])
        }
        let targetHidden = MLXRandom.normal([1, spec.rows, concatDim]).asType(pairing.target)
        let block = MLXArray(
        ...
        for state in [stateA, stateB] {
            ...
            #expect(state.contextCaches[0].keys?.dtype == pairing.drafter, "round \(round)")
